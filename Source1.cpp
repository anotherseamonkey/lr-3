...
void saveTime(const string name, double time) {
	ofstream fout(name);
	fout << time << endl;
	fout.close();
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2)
    {
	cout << "Program usage: " << endl <<"./" << argc << " <n>" << endl << "where <n> is the size of square matrix" << endl;
	return -1;
    }
	...
	parallel = end - beg;
	saveTime(format("Time (num of threads = {}), size = {}", n, N), parallel);
	cout << endl << "Calculation time: " << parallel << " seconds" << endl;
